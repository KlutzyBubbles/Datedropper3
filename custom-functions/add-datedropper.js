(function($) {
	...
	var // CSS EVENT DETECT
		...
		picker_hide = function() {
			if (!is_locked()) {
				...
				$('input[data-id="' + picker.element.attr('id') + '"]').blur();
				if (picker.element.hasClass('picker-modal')) {
		...
		picker_render_calendar = function() {
			...
			var _S = new Date(get_current_full()),
		...
		picker_ul_transition = function(k, i) {
			...
			var li;
			if (i === get_eq(k, 'last')) {
				li = ul.find('li[value="' + get_eq(k, 'first') + '"]');
				li.clone().insertAfter(ul.find('li[value=' + i + ']'));
				li.remove();
			}
			if (i === get_eq(k, 'first')) {
				li = ul.find('li[value="' + get_eq(k, 'last') + '"]');
	...
	$(document)
		...
		.on(ui_event.m, function(e) {
			...
			if (pick_dragged) {
				...
				o = Math.round(o * 0.026);
				var int = get_clear(k, pick_drag_temp + o);
		...
		$(this).each(function() {
			if ($(this).is('input') && !$(this).hasClass('picker-input')) {
				...
				for (var k in pickers[id].key) {
					if (pickers[id].key.hasOwnProperty(k)) {
						$('<ul>', {
							class: 'pick pick-' + k,
							'data-k': k
				...
				if (pickers[id].large) {
					...
					for (var j = 0; j < 7; j++) {
						$('<li>', {
							html: i18n[pickers[picker.id].lang].weekdays.short[picker_day_offset[j]]
	...
	$.fn.getDateUnformatted = function() {
		if ($(this).data('id')) {
			picker = { id: $(this).data('id') };
			return get_current_full();
		} else {
			return $(this).val();
		}
	};
